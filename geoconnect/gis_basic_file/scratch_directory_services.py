...
class ScratchDirectoryHelper:
    ...
    @staticmethod
    def delete_scratch_work_directory(gis_file_helper):
        """If scratch directory exists, delete it
        return False    gis_file_helper is None 
        return True     directory doesn't exist 
        return True     directory is deleted
        return False    failed to delete directory
        """
        if gis_file_helper is None:
            logger.error('GIS file helper is None')
            return False        
            
        if not ScratchDirectoryHelper._does_scratch_work_directory_exist(gis_file_helper):
            return True
        
        try:
            shutil.rmtree(gis_file_helper.gis_scratch_work_directory)
            return True
        except:
            logger.error('Failed to delete directory: %s:' % gis_file_helper.gis_scratch_work_directory) 
            return False
    
    @staticmethod
    def get_scratch_work_directory(gis_file_helper):
        ...
        if ScratchDirectoryHelper._does_scratch_work_directory_exist(gis_file_helper):
            return gis_file_helper.gis_scratch_work_directory
            
        # If directory doesn't exist, create it
        if ScratchDirectoryHelper._make_scratch_work_directory(gis_file_helper):
            return gis_file_helper.gis_scratch_work_directory
        
        return None
    
    @staticmethod
    def _does_scratch_work_directory_exist(gis_file_helper):
        ...
        if gis_file_helper.gis_scratch_work_directory and \
            os.path.isdir(gis_file_helper.gis_scratch_work_directory):
            return True
        return False
         
         
    @staticmethod
    def _make_scratch_work_directory(gis_file_helper):
        """For a GISFileHelper object, create a scratch directory.
        ...
        if not gis_file_helper.id:
            gis_file_helper.save()  # save to create id
        ...
        try:
            os.makedirs(dirname)
            gis_file_helper.gis_scratch_work_directory = dirname
            gis_file_helper.save()
            return True
        except:
